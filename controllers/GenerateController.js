...
const terimakasih = async (req, res, next) => {
  try {
    ...
    doc.text("Surat ini dilampirkan sebagai bukti sah telah dilaksanakannya proses voting pada SISTEM BPS Kota Padang.");
    doc.moveDown();

    doc.text(`Padang, ${generateTanggal.toLocaleDateString()}`);
    doc.text("Sistem SIVOTING BPS Kota Padang");
    doc.moveDown();
